...
while:
##FIX: something along lines of, if address of arr[j-1] is less than start arr addr, 
   ##check arr[j] against arr[j+1] and then switch them accordingly???
...
# $t3 : array[j+1] OOPS, get j-1 ##FIX
add   $t2, $t2, 4 #+4 if it fails
lb    $t4, 0($t2)

#conditional jump to swapVals: could do by "if not j>0, break. if not arr[j] < arr[j-1], break"
blez $t7, forLoop
bge  $t4, $t3, forLoop #arr[j-1], arr[j] - switched operands fixed backwards sort issue
#addi $t6, $t6, 1  # i=i+1
j swapVals
